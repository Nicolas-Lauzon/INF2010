...
public final class PointOperator {
    ...
    public static Point2d getMinCoord(Collection<Point2d> coords) {
        ...
        if(!coords.isEmpty()) {
            coordXmin = Collections.min(coords.stream().map(Point2d::X).collect(Collectors.toList()));
            coordYmin = Collections.min(coords.stream().map(Point2d::Y).collect(Collectors.toList()));
        }
